...
st.title("UniSwapV3 Simulated Returns, LP hedging with knockin put option, enter LP at max price at 100% USD")
...
if P_max > P:
    ...
    #Set your knock in barrier (set to lower bound P_min to exactly hedge the left tail)
    B_knockin = col3.number_input('Down and In barrier', value=P_min, step = 0.001)
    ...
    pnl_hedged_put_vanilla = []
    pnl_hedged_put_knockin =[]
    for price in prices:
        x_future = max(0,L*(min(1/np.sqrt(P_min),1/np.sqrt(price))-1/np.sqrt(P_max)))
        y_future = max(0,L*(min(np.sqrt(P_max),np.sqrt(price))-np.sqrt(P_min)))
        pnl_hedged_put_vanilla.append(y_future + x_future * price  - y + x_min_new * max(0,P_put_new-price))
        pnl_hedged_put_knockin.append(y_future + x_future * price  - y + x_min_new * (P_put_new-price if price <= B_knockin else 0))

    df = pd.DataFrame()
    df["Price"] = prices
    df["PNL Vanilla Put Hedge"] = pnl_hedged_put_vanilla
    df["PNL Knockin Put Hedge"] = pnl_hedged_put_knockin

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df["Price"], y=df["PNL Vanilla Put Hedge"], mode='lines', name="Vanilla Put Hedge"))
    fig.add_trace(go.Scatter(x=df["Price"], y=df["PNL Knockin Put Hedge"], mode='lines', name="Knockin Put Hedge"))

    fig.update_layout(title='Impermanent loss, LP with put hedging<br>Number of puts: ' + str(x_min_new) + ', Strike = ' + str(P_put_new) + ', Down and in barrier = ' + str(B_knockin) + '<br>Notional protection: ' + str(x_min_new * P_put_new) )
    fig.update_xaxes(title_text='Price')
    fig.update_yaxes(title_text='PNL')
    ...
    with st.form("generate_mc_simulaton"):
        ...
        if submitted:
            ...
            #y_future + x_future * price  - y = impermanent loss
            #x_min_new * max(0,P_put_new-price) = put payoff
            #mc_paths["earnings"]  = generated fees
            mc_paths["PnL Vanilla Put Hedge"] = pnl_hedged_put_vanilla + mc_paths["earnings"]    

            col1, col2, col3 = st.columns([2,1,1])
            df = px.data.tips()
            fig = px.histogram(mc_paths["PnL Vanilla Put Hedge"], title = "PnL including generated fees, hedged with " + str(x_min_new) + " vanilla puts, <br>Strike: " +str(P_put_new) + "<br>Number of simulations: " + str(number_paths))
            col1.plotly_chart(fig,use_container_width=True)

            col2.text("Average PnL, hedged LP with vanilla put: ") 
            col3.text(mc_paths["PnL Vanilla Put Hedge"].mean())
            col2.text("Median PnL, hedged LP with vanilla put: ") 
            col3.text(mc_paths["PnL Vanilla Put Hedge"].median())
            col2.text("5th percentile PnL, hedged LP with vanilla put: ") 
            col3.text(mc_paths["PnL Vanilla Put Hedge"].quantile(.05))
            col2.text("1st percentile PnL, hedged LP with vanilla put: ") 
            col3.text(mc_paths["PnL Vanilla Put Hedge"].quantile(.01))        
            ...
            #y_future + x_future * price  - y = impermanent loss
            #x_min_new * (P_put_new-price if price <= B_knockin else 0) = knockin put payoff
            #mc_paths["earnings"]  = generated fees
            mc_paths["PnL Knockin Put Hedge"] = pnl_hedged_put_knockin + mc_paths["earnings"]            

            col1, col2, col3 = st.columns([2,1,1])
            df = px.data.tips()
            fig = px.histogram(mc_paths["PnL Knockin Put Hedge"], title = "PnL including generated fees, hedged with " + str(x_min_new) + " knockin puts, <br>Strike: " +str(P_put_new) + ', Down and in barrier = ' + str(B_knockin) + "<br>Number of simulations: " + str(number_paths))
            col1.plotly_chart(fig,use_container_width=True)

            col2.text("Average PnL, hedged LP with knockin put: ") 
            col3.text(mc_paths["PnL Knockin Put Hedge"].mean())
            col2.text("Median PnL, hedged LP with knockin put: ") 
            col3.text(mc_paths["PnL Knockin Put Hedge"].median())
            col2.text("5th percentile PnL, hedged LP with knockin put: ") 
            col3.text(mc_paths["PnL Knockin Put Hedge"].quantile(.05))
            col2.text("1st percentile PnL, hedged LP with knockin put: ") 
            col3.text(mc_paths["PnL Knockin Put Hedge"].quantile(.01))                
            ...
            for i in range(0, len(mc_paths)):
                price = float(mc_paths["Price End"][i])
                values_put_vanilla.append(max(0,P_put_new-price))
                values_put_knockin.append(P_put_new-price if price <= B_knockin else 0)
                
            mc_paths["Payoff Vanilla Put"] = values_put_vanilla
            mc_paths["Payoff Knockin Put"] = values_put_knockin
            ...
            df_cost_of_hedge = pd.DataFrame([["Vanilla put", euro_option(P, P_put_new, T, r, sigma, 'put'), x_min_new, x_min_new*euro_option(P, P_put_new, T, r, sigma, 'put')], 
                                            ["Knockin put", (P_put_new - B_knockin) * euro_option(P, B_knockin, T, r, sigma, 'binary put') + euro_option(P, B_knockin, T, r, sigma, 'put'), x_min_new, x_min_new*((P_put_new - B_knockin) * euro_option(P, B_knockin, T, r, sigma, 'binary put') + euro_option(P, B_knockin, T, r, sigma, 'put'))]],
                                            columns = ["Hedging instrument", "Theoretical price", "Number of instruments", "Theoretical hedging cost"])
            st.dataframe(df_cost_of_hedge)

            df_put_prices = pd.DataFrame([["Vanilla put", euro_option(P, P_put_new, T, r, sigma, 'put'), np.exp(-r*T)*np.mean(values_put_vanilla)], 
                                            ["Knockin put", (P_put_new - B_knockin) * euro_option(P, B_knockin, T, r, sigma, 'binary put') + euro_option(P, B_knockin, T, r, sigma, 'put'),np.exp(-r*T)*np.mean(values_put_knockin)]],
